...
- **工作组管理**：创建/层级化工作组，归类整理大量标签页；管理面板左侧工作组树支持拖拽调整层级与同级顺序（顺序保存在后端，多设备一致）；右键或「⋮」菜单可**建立副本**（递归复制整棵子树：所有子/孙工作组与标签页、标签关联，副本以「源名称 (副本)」命名并作为同级节点追加到末尾）
- **标签系统**：为工作组或组内标签页打标签（支持自定义颜色），可按标签筛选，后端存储、跨设备共享
- **多设备同步**：通过轻量后端实现标签页在多浏览器/多设备间同步
- **快捷键「加入并关闭」**：在设置中选择「默认收藏工作组」后，按 `Shift+Alt+S` 即可将当前标签页收藏进该工作组并自动关闭（未设置默认工作组时打开侧边栏引导选择；是否启用该快捷键可在设置中开关）
...
src/
  ...
  dashboard/             # 完整管理面板（标签页/工作组/设备/设置）
  shared/
