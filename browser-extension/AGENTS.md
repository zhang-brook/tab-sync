...
Chrome 浏览器标签页管理扩展，支持标签页同步、工作组管理、多设备协作。
...
| 陷阱 | 正确做法 |
|------|----------|
| Service Worker 30s 无活动可能被终止 | 所有状态持久化到 `chrome.storage.local`，不依赖内存变量 |
...
- 版本协商通过 `CHECK_VERSION` 消息触发，请求后端 `/v1/tab-sync/version`
- **401 处理**: Token 无效时直接清除认证状态（无 refresh token 续签）
...
src/
  background/
    index.ts              # SW 入口，消息监听 + 侧边栏配置 + 快捷键命令
    message-handler.ts    # 统一消息分发器 (所有 UI 消息处理)
  sidepanel/              # 侧边栏面板 (标签页快捷同步)
  dashboard/              # 独立标签页管理面板 (完整功能 SPA)
    views/
      DashboardView.vue   # 仪表盘概览
      TabsView.vue        # 本地标签页管理 (搜索/筛选/批量操作)
      SyncedView.vue      # 已同步标签页
      WorkspacesView.vue  # 工作组管理 (CRUD + 恢复打开)
      TagsView.vue        # 标签管理
      RecycleBinView.vue  # 回收站
      DevicesView.vue     # 多设备概览
      SettingsView.vue    # 设置 (认证/连接/数据)
  shared/
    types/                # TypeScript 类型定义
    api/                  # fetch 封装 + 各模块 API
    composables/          # Vue composables (useMessage 等)
    storage/              # chrome.storage.local 封装
    utils/                # 工具函数 (UUID, 设备指纹, 工作组树, 日志)
    components/           # 跨页面共享组件
```
...
- CSS 使用 `<style scoped>`，不使用 CSS 预处理器

### 5.4 后端状态

- 轻量后端已实现（Go + Gin + SQLite），支持设备注册、工作组 CRUD、标签、回收站、API Key 认证
- 后端启动后在浏览器访问 `/setup` 完成首次设置，获取 Admin Token
- API 调用包裹 try-catch，失败时仅记录日志不影响本地功能
- 后端代码位于仓库根目录 `server/` 下

---

## 6. 已知边界情况

1. **`chrome.tabs.get()` 的竞态**: 在检测标签页是否存在和实际操作之间，标签页可能被用户关闭。所有 Chrome API 调用都应包裹 try-catch。
2. **Prettier 自动格式化**: 编辑器保存时可能自动格式化代码，导致"File has been modified since read"错误。编辑前应先重新读取文件。
...
4. 验证: SidePanel 可打开、Dashboard 可在新标签页中打开
5. 操作标签页与工作组，观察 Service Worker 控制台日志

---

## 8. 后续开发建议

- 可优化项: 扩展图标 Badge 显示同步状态
